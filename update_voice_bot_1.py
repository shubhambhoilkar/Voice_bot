...
def listener_loop():
    ...
    while True:
        if not audio_q.empty():
            ...
            chunk = aec_process(chunk)  # Apply echo cancellation before VAD

            #added code fro recognising self voice
            if is_playing_ai and interrupted_audio_data is not None:
                try:
                    # Read AI playback audio (float32, not "float2")
                    ai_audio, sr = sf.read(io.BytesIO(interrupted_audio_data), dtype='float32')

                    # Ensure same channel dimension
                    if ai_audio.ndim > 1:
                        ai_audio = ai_audio.mean(axis=1)
                    ...
                    if np.max(np.abs(chunk)) > 0:
                        mic_chunk = chunk / np.max(np.abs(chunk))
                    else:
                        mic_chunk = chunk

                    # Compute cross-correlation similarity
                    corr = np.correlate(mic_chunk, ai_snippet, mode='valid')
                    corr_value = float(np.max(corr))

                    if corr_value > 0.7:
                        print(f"Echo detected (corr={corr_value:.2f}), skipping this frame.")
                        continue

                except Exception as e:
                    print("Echo detection error:", e)
                #end of Echo Code
