...
def get_response_from_ai(text):
    ...
    print(f"🛰️ API called with: {text!r} | is_responding={is_responding}")
...
def listener_loop():
    ...
    global speech_start_time  # NEW: For accurate speech duration
    while True:
        if not audio_q.empty():
            chunk = audio_q.get().flatten()
            try:
                speech = vad_iterator(torch.from_numpy(chunk), return_seconds=False)
            except Exception as e:
                print("VAD error:", e)
                speech = False
            ...
            if speech:
                ...
                if not is_talking:
                    ...
                    is_talking = True
                    speech_start_time = time.time()  # NEW: Start timing actual speech
                    if is_playing_ai:
                        interrupted_pos = pygame.mixer.music.get_pos()
                        pygame.mixer.music.stop()
                        is_playing_ai = False
                        print("⏸️ AI interrupted — waiting to see if user actually speaks...")
                last_speech_time = time.time()
            else:
                silence_counter += 1
                # Detect speech end
                if (
                    ...
                    and (time.time() - last_speech_time) > (VAD_SILENCE_TIMEOUT + SPEECH_END_PADDING)
                ):
                    # NEW: Accurate dur (speech time only, ignores trailing silence)
                    ...
                    is_talking = False
                    speech_buffer = np.zeros(0, dtype=np.float32)
                    speech_start_time = None  # NEW: Reset
                # Resume if false interrupt (short/noise)
                elif (
                    not is_talking
                    and not is_playing_ai
                    and interrupted_audio_data is not None  # Note: 'is not None'
                    and (time.time() - last_speech_time) > 1.5
                ):
